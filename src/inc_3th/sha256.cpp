...
//    printf("\n Initalized variables... Entering loops\n");
    ...
    char cbuf[256] = {0};
    std::string hash;
    sprintf(cbuf, "%08llx", H[0]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[1]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[2]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[3]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[4]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[5]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[6]);
    hash += cbuf;

    sprintf(cbuf, "%08llx", H[7]);
    hash += cbuf;
